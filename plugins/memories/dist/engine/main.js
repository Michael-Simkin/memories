...
async function appendOperationLog(path5, payload) {
  await appendJsonLine(path5, redactUnknown(payload));
}
async function readJsonLogs(path5, limit = 200) {
  try {
...
var MemoryStore = class {
  db;
  vecEnabled;
  constructor(dbPath, vecExtensionPath2) {
    ...
    this.vecEnabled = this.tryEnableVec(vecExtensionPath2 ?? null);
    this.initialize();
  }
  close() {
    this.db.close();
...
function createEngineApp(options) {
  ...
  const store = new MemoryStore(
    path2.join(options.projectRoot, ".memories", "ai_memory.db"),
...
function resolvePluginRoot() {
  ...
  const moduleDir = path3.dirname(currentFilePath);
...
function vecExtensionPath(pluginRoot) {
  return path4.join(pluginRoot, "native", "node_modules", vecPlatformPackage(), "vec0");
}
async function ensureSqliteVec(pluginRoot) {
  if (existsSync2(vecBinaryPath(pluginRoot))) {
    return vecExtensionPath(pluginRoot);
  }
  const pkgSpec = `${vecPlatformPackage()}@${SQLITE_VEC_VERSION}`;
  const prefixDir = path4.join(pluginRoot, "native");
  try {
